# Zmanim Documentation

This site explains common zmanim in plain language. Each page describes what a zman represents and, where available, the basic approach used to calculate it.

You may have reached this site because a zmanim app you use linked you here for more detail about a particular time. These pages are meant to help you understand the time shown in your app, including any sources, offsets, or calculation methods that may affect it.

Note that different apps may use different versions, settings, or defaults, so the information here should be used as a guide rather than a guarantee that every app calculates a zman in exactly the same way.

## Categories

- [Alos HaShachar](alos.md)
- [Astronomical Dawn](astronomical_dawn.md)
- [Misheyakir](mishayakir.md)
- [Netz HaChamah](sunrise.md)
- [Sof Zman Shema](end_shema.md)
- [Sof Zman Tefilla](end_tefilla.md)
- [Sof Zman Achilas Chametz](end_eating_chometz.md)
- [Sof Zman Biur Chametz](end_burning_chometz.md)
- [Chatzos Hayom](midday.md)
- [Mincha Gedola](mincha_gedola.md)
- [Mincha Ketana](mincha_ketana.md)
- [Plag Mincha](plag_mincha.md)
- [Candle Lighting](candle_lighting.md)
- [Sunset](sunset.md)
- [Bein Hashmashos](bein_hashmashos.md)
- [Tzais Geonim](tzais_geonim.md)
- [Tzais](tzais.md)
- [Astronomical Twilight](astronomical_twilight.md)
- [Midnight](midnight.md)
- [Kidush Levana](kidush_levana.md)
